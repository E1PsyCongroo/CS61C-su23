...
class TestAbsLoss(unittest.TestCase):
    def test_abs_loss_standard(self):
        ...
        array0 = t.array(list(range(1, 10)))
        # load address of `array0` into register a0
        t.input_array("a0", array0)
        # create array1 in the data section
        array1 = t.array([1, 6] * 4 + [1])
        ...
        t.check_scalar("a0", sum([0, 4, 2, 2, 4, 0, 6, 2, 8]))
        # generate the `assembly/TestAbsLoss_test_simple.s` file and run it through venus
        t.execute()

    def test_abs_loss_invalid_n(self):
        # load the test for abs_loss.s
        t = AssemblyTest(self, "../coverage-src/abs_loss.s")
        t.input_scalar("a2", 0)
        t.call("abs_loss")
...
class TestSquaredLoss(unittest.TestCase):
    ...
    def test_squared_loss_invalid_n(self):
    ...
    def test_squared_loss_standard(self):
        ...
        t.call("squared_loss")
        # check that the result array contains the correct output
        t.check_array(result, [i ** 2 for i in [0, 4, 2, 2, 4, 0, 6, 2, 8]])
        # check that the register a0 contains the correct output
        t.check_scalar("a0", sum([i ** 2 for i in [0, 4, 2, 2, 4, 0, 6, 2, 8]]))
...
class TestInitializeZero(unittest.TestCase):
    ...
    def test_initialize_zero_too_big(self):
        t = AssemblyTest(self, "../coverage-src/initialize_zero.s")
        # input the length of the desired array
        t.input_scalar("a0", 0xffff)
        # call the `initialize_zero` function
        t.call("initialize_zero")
